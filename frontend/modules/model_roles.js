...
function renderEvolveRoleSummary() {
  ...
  el.innerHTML = `<span>${escHtml(planner)}</span><span>${escHtml(executor)}</span><button type="button" onclick="showPanel('roles')">Change in Role Matrix</button>`;
}
  ...
  for (const role of MODEL_ROLE_DEFS) {
    ...
    card.innerHTML = `
      ...
      <div class="role-card-control">
        <select class="role-select" aria-label="${escHtml(role.label)} model" onchange="setModelRole('${escHtml(role.id)}', this.value)">
