...
async function testEndpoint() {
  ...
  if (status) status.innerHTML = '<span class="status-offline" style="color:var(--yellow)">Testing…</span> querying provider API';
  ...
  for (const ep of addedEndpointsList) {
    ...
    card.innerHTML = `
      <div class="endpoint-card-main">
        <div class="endpoint-card-info" onclick="toggleEndpointModels('${escHtml(ep.id)}')">
        ...
        <div class="endpoint-card-actions">
          <button class="btn-endpoint-disable" onclick="toggleEndpointStatus('${escHtml(ep.id)}')">${isEnabled ? 'Disable' : 'Enable'}</button>
          <button class="btn-endpoint-delete" onclick="deleteEndpoint('${escHtml(ep.id)}')">Delete</button>
          <button class="btn-endpoint-expand" onclick="toggleEndpointModels('${escHtml(ep.id)}')">˅</button>
        </div>
      </div>
      <div class="endpoint-models-drawer" id="drawer-${escHtml(ep.id)}" style="display:none;">
        <div class="drawer-toolbar">
          <div class="drawer-bulk">
            <button onclick="setAllEndpointModels('${escHtml(ep.id)}', true)">Enable All</button>
            <button onclick="setAllEndpointModels('${escHtml(ep.id)}', false)">Disable All</button>
          </div>
        </div>
...
function renderDrawerGrid(ep, filterQuery) {
  ...
  for (const m of models) {
    ...
    item.innerHTML = `
      <input type="checkbox" ${isChecked ? 'checked' : ''} onchange="toggleSingleEndpointModel('${escHtml(ep.id)}', '${escHtml(m.id)}', this.checked)" />
...
async function toggleSingleEndpointModel(id, modelId, isChecked) {
  ...
  try {
    ...
    // Update badge quietly without re-rendering drawer
...
/* Backwards compatibility helpers/stubs for legacy bootstrap references */
