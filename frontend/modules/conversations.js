...
function loadConversation(id) {
  ...
  if (!conv) return;
  // Open as floating panel instead of rendering into static messages div
  if (typeof openConversationAsPanel === 'function') {
    openConversationAsPanel(id);
  }
}
...
function renderConvList() {
  ...
  for (const c of filtered) {
    ...
    el.innerHTML = `<span class="conv-item-title">${escHtml(c.title)}</span>
      <button class="conv-rename-btn" data-action="rename-conv" data-conv-id="${escHtml(c.id)}" title="Rename" aria-label="Rename conversation">✎</button>
      <button class="conv-delete" data-action="delete-conv" data-conv-id="${escHtml(c.id)}" title="Delete" aria-label="Delete conversation">✕</button>`;
    el.addEventListener('click', (e) => {
      const actionBtn = e.target.closest('button[data-action]');
      if (actionBtn) {
        const action = actionBtn.dataset.action;
        if (action === 'rename-conv') startRenameConversation(c.id, e);
        else if (action === 'delete-conv') deleteConversation(c.id, e);
        return;
      }
